...
const { loading, success, error, closeAll } = useMessage()
...
const columns = computed(() => {
  const base = [
    ...
    { label: '华为合入PR', key: 'hwMergePR', width: 200, minWidth: 200 },
...
const emailBindDialog = reactive({ visible: false })
const branchDialog = reactive({ visible: false, branch: '' })
...
const handleBranchConfirm = () => {
  if (!branchDialog.branch) { OMessage.warning('请选择目标分支'); return }
  branchDialog.visible = false
  const selectedList = patches.value.filter(p => selectedIds.value.includes(p.id))
  const closeLoading = loading('补丁PR创建中...')
  setTimeout(() => {
    closeLoading()
    selectedList.forEach(p => {
      p.hwMergePR = `https://gitcode.com/openeuler/kernel/pull/${Math.floor(Math.random() * 20000)}`
    })
    success(`${selectedList.length} 个补丁合入成功`)
  }, 1500)
  emailBindDialog.visible = true
}
...
const handleEditConfirm = () => {
  if (!editDialog.form.mainKey.trim()) { OMessage.warning('唯一标识符不能为空'); return }
  if (!editDialog.form.title.trim()) { OMessage.warning('概述(SR粒度)不能为空'); return }
  if (!editDialog.form.description.trim()) { OMessage.warning('功能介绍不能为空'); return }
  if (!editDialog.form.patchType) { OMessage.warning('请选择补丁类型'); return }
  if (!editDialog.form.productVersion) { OMessage.warning('请选择产品版本'); return }
  if (!editDialog.form.userKernel) { OMessage.warning('请选择用户态/内核态'); return }
  if (!editDialog.form.patchModule) { OMessage.warning('请选择补丁模块'); return }
  if (!editDialog.form.commitOE.trim()) { OMessage.warning('Commit OE不能为空'); return }
  if (!editDialog.form.commitIdOE.trim()) { OMessage.warning('Commit ID OE不能为空'); return }
  if (!editDialog.form.oeMergeTag.trim()) { OMessage.warning('OE Merge tag不能为空'); return }
...
const handleAddConfirm = () => {
  ...
  if (!addDialog.form.description.trim()) { OMessage.warning('功能介绍不能为空'); return }
  ...
  if (!addDialog.form.userKernel) { OMessage.warning('请选择用户态/内核态'); return }
  if (!addDialog.form.patchModule) { OMessage.warning('请选择补丁模块'); return }
  if (!addDialog.form.commitOE.trim()) { OMessage.warning('Commit OE不能为空'); return }
  if (!addDialog.form.commitIdOE.trim()) { OMessage.warning('Commit ID OE不能为空'); return }
  if (!addDialog.form.oeMergeTag.trim()) { OMessage.warning('OE Merge tag不能为空'); return }
...
<template>
  <div class="patch-tab">
    ...
    <div class="patch-tab__repo-row">
      <span class="patch-tab__repo-label">目标仓库<span class="info-tip-wrapper"><OIconInfoTip style="margin-left: 4px; cursor: pointer; vertical-align: middle;" /><span class="info-tip-text">目标仓库必须为公开仓库</span></span></span>
    ...
    <div class="patch-tab__table-wrap">
      <OTable :columns="columns" :data="pagedRows">
        ...
        <template #td_hwMergePR="{ row }">
          <OLink v-if="row.hwMergePR" color="primary" :href="row.hwMergePR" target="_blank" class="patch-tab__link">
            {{ row.hwMergePR }}
            <template #suffix>
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M10 2H14V6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M14 2L9 7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M13 9V13C13 13.5304 12.7893 14.0391 12.4142 14.7893 11.5304 15 11 15H3C2.46957 15 1.96086 14.7893 1.58579 14.4142C1.21071 14.0391 1 13.5304 1 13V5C1 4.46957 1.21071 3.96086 1.58579 3.58579C1.96086 3.21071 2.46957 3 3 3H7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </template>
          </OLink>
          <span v-else class="patch-tab__muted">—</span>
  ...
  <ODialog v-model:visible="editDialog.visible" title="编辑补丁" size="large">
    <div class="patch-form">
      ...
      <div class="patch-form__grid">
        ...
        <div class="patch-form__field">
          <label class="patch-form__label"><span class="patch-form__required">*</span>Commit ID OE</label>
  ...
  <!-- ══ 邮箱绑定提示弹窗 ════════════════════════════════════════════════════════ -->
  <ODialog v-model:visible="emailBindDialog.visible" title="绑定邮箱" size="small">
    <div class="batch-delete-body">
      <p class="batch-delete-body__text">
        合入补丁需要绑定邮箱，您当前尚未绑定邮箱。请前往个人中心完成邮箱绑定。
      </p>
    </div>
    <template #footer>
      <div class="patch-dialog-footer">
        <OButton variant="outline" size="medium" @click="emailBindDialog.visible = false">稍后再说</OButton>
        <OButton variant="solid" color="primary" size="medium" @click="emailBindDialog.visible = false; router.push('/profile')">前往个人中心</OButton>
      </div>
    </template>
  </ODialog>

  <!-- ══ 选择目标分支弹窗 ════════════════════════════════════════════════════════ -->
  <ODialog v-model:visible="branchDialog.visible" title="选择目标分支" size="small">
    <div class="patch-form">
      <div class="patch-form__field">
        <label class="patch-form__label">目标分支</label>
        <OSelect v-model="branchDialog.branch" placeholder="请选择目标分支">
          <OOption value="master" label="master" />
          <OOption value="openEuler-24.03-LTS" label="openEuler-24.03-LTS" />
          <OOption value="openEuler-22.03-LTS-SP3" label="openEuler-22.03-LTS-SP3" />
          <OOption value="dev" label="dev" />
        </OSelect>
      </div>
    </div>
    <template #footer>
      <div class="patch-dialog-footer">
        <OButton variant="outline" size="medium" @click="branchDialog.visible = false">取消</OButton>
        <OButton variant="solid" color="primary" size="medium" @click="handleBranchConfirm">确认合入</OButton>
      </div>
    </template>
  </ODialog>
...
.info-tip-wrapper .info-tip-text {
  visibility: hidden;
  opacity: 0;
  position: absolute;
  bottom: 130%;
...
.info-tip-wrapper:hover .info-tip-text {
  visibility: visible;
  opacity: 1;
}
</style>
