...
const pipelineColumns = computed(() => [
  ...
  { label: t('pipeline.colAction'), key: 'action', style: { width: '160px', minWidth: '160px' } },
])
...
const cancelPipeline = (row: any) => {
  row.pipelineStatus = 'cancelled'
  row.testStatus = 'cancelled'
  const now = new Date()
  row.endedAt = `${now.getFullYear()}.${now.getMonth()+1}.${now.getDate()} ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`
  row.duration = '—'
  OMessage.success(`流水线 ${row.taskId} 已取消执行`)
}
        ...
        <div class="proj-detail__overview">
          ...
          <OCard class="proj-detail__form-card">
            ...
            <div class="proj-detail__info-grid">
              ...
              <div class="proj-detail__info-field">
                <label class="proj-detail__info-label">目标仓库<span class="info-tip-wrapper"><OIconInfoTip style="margin-left: 4px; cursor: pointer; vertical-align: middle;" /><span class="info-tip-text">目标仓库必须为公开仓库</span></span></label>
            ...
            <OTable :columns="pipelineColumns" :data="pagedPipelineTasks">
              ...
              <template #td_action="{ row }">
                <div class="pl-action-cell">
                  ...
                  <OButton variant="outline" color="danger" size="small" round="pill" :disabled="row.pipelineStatus !== 'running'" @click="cancelPipeline(row)">{{ t('pipeline.cancel') }}</OButton>
                </div>
              </template>
            </OTable>
            </div>
      ...
      <div class="edit-proj-form">
        ...
        <div class="edit-proj-form__grid">
          ...
          <div class="edit-proj-form__field">
            ...
            <OSelect v-model="editForm.modules" multiple :max-tag-count="3" clearable :before-select="handleEditModuleBeforeSelect">
          ...
          <div class="edit-proj-form__field edit-proj-form__field--full">
            <label class="edit-proj-form__label"><span class="edit-proj-form__required">*</span>目标仓库<span class="info-tip-wrapper"><OIconInfoTip style="margin-left: 4px; cursor: pointer; vertical-align: middle;" /><span class="info-tip-text">目标仓库必须为公开仓库</span></span></label>
        ...
        <div class="add-proj-form__grid">
          ...
          <div class="add-proj-form__field add-proj-form__field--full">
            <label class="add-proj-form__label">
              ...
              <span class="info-tip-wrapper"><OIconInfoTip style="margin-left: 4px; cursor: pointer; vertical-align: middle;" /><span class="info-tip-text">目标仓库必须为公开仓库</span></span>
            </label>
