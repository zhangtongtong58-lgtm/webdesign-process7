...
const getCellValue = ({ row, column }) => {
  return helper.getValueByPath(row, column.key);
};
const isEmptyCell = (cellValue) => {
  return is.isNil(cellValue) || cellValue.toString() === "";
};
...
const getColumnCount = (columns) => {
  ...
  const traverseColumns = (_columns) => {
    if (!is.isArray(_columns) || !_columns.length) {
      return;
    }
...
const markHeaderHidden = (groupColumns) => {
  groupColumns.forEach((groupColumn) => {
    for (let colIndex = 0; colIndex < groupColumn.length; colIndex++) {
      ...
      if (is.isNil(column.customColSpan) || column.customColSpan < 2) {
        continue;
      }
...
const getGroupColumns = (options) => {
  ...
  const traverseColumns = (traverseOptions) => {
    ...
    traverseOptions.columns.forEach((column) => {
      ...
      if (is.isArray(cell.children) && cell.children.length) {
...
const getIsLevelExpandable = ({
  ...
  if (!is.isArray(list) || !list.length) {
  ...
  list.forEach((child, childIndex) => {
    if (is.isArray(child.children) && !!child.children.length || child.hasChildren) {
      expandable = true;
...
exports.getIsLevelExpandable = getIsLevelExpandable;
exports.getTotalHeaderRows = getTotalHeaderRows;
