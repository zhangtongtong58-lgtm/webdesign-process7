...
class Gallery extends effect {
  constructor(slideElList, slideContainer, activeIndex, options) {
    ...
    const or = useResizeObserver.useResizeObserver();
    const listener = helper.debounceRAF(() => {
  ...
  active(toIndex, animate = true, force = false) {
    ...
    if (this.currentIndex !== toIndex && is.isFunction(this.onBeforeChange) && this.onBeforeChange(toIndex, this.currentIndex) === false) {
    ...
    if (this.alignType === "center") {
      return this.transformX((cw - sw) / 2 - sl, animate).then(() => {
        if (is.isFunction(this.onChanged) && this.currentIndex !== toIndex) {
  ...
  destroyed() {
    if (is.isFunction(this.destroyObserver)) {
      this.destroyObserver();
    }
  }
}
